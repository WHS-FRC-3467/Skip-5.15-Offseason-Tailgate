...
public class IntakeConstants {
    ...
    public static Intake get()
    {
        switch (Constants.currentMode) {
            case REAL:
                return new Intake(new FlywheelMechanismReal(NAME,
                    new MotorIOTalonFX(NAME, getFXConfig(), Ports.intake, new TalonFXFollower(
                        Ports.flywheelFollower, false))));
            case SIM:
                return new Intake(new FlywheelMechanismSim(NAME,
                    new MotorIOTalonFXSim(NAME, getFXConfig(), Ports.intake,
                        new TalonFXFollower(
                            Ports.flywheelFollower, false)),
                    DCMOTOR, MOI, TOLERANCE));
            case REPLAY:
                return new Intake(new FlywheelMechanism() {});
            default:
